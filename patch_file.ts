    ...
    reader.onload = async (evt) => {
      try {
        ...
        for (let i = 0; i < data.length; i++) {
          ...
          let matchedModel = String(row['Evaluation Workflow'] || '').trim();
          ...
          const emp = {
            ...
            khmerName: String(row['Khmer Name'] || '').trim(),
            campus: String(row['Campus'] || '').trim(),
            department: String(row['Department'] || '').trim(),
            position: String(row['Position'] || '').trim(),
            category: String(row['Category'] || '').trim(),
            supervisorId: String(row['Direct Supervisor'] || row['Direct Supervisor ID'] || '').trim(),
            supporterId: String(row['Supporter'] || row['Supporter ID'] || '').trim(),
            ...
            evalPeriod: String(row['Evaluation Period'] || '').trim(),
